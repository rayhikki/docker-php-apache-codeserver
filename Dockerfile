...
# 2. Configure Locale & Set Timezone
# Generate both en_US.UTF-8 and en_GB.UTF-8 locales to prevent warnings.
RUN sed -i -e '/en_US.UTF-8/s/^# //g' -e '/en_GB.UTF-8/s/^# //g' /etc/locale.gen && locale-gen
...
RUN docker-php-ext-install pdo pdo_mysql pdo_pgsql pcntl snmp zip bcmath sockets
...
# 7. Add and configure the entrypoint script
COPY entrypoint.sh /usr/local/bin/
COPY user.env /usr/local/bin/
RUN chmod +x /usr/local/bin/user.env
# RUN useradd -u 2000 -m -s /bin/bash -g www-data -G sudo $SSH_USER
