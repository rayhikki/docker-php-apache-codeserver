...
RUN echo "alias ll='ls -alhF'" >> /root/.bashrc && \
    echo "alias lll='ls -alhF --time-style=long-iso'" >> /root/.bashrc


# ==> Configure SSH <==
# 1. Create a directory for the SSH daemon to run in.
# 2. Permit the root user to log in via SSH. This is generally not recommended
#    for production but is convenient for development environments.
# RUN mkdir /var/run/sshd
RUN sed -i 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config

# ==> Set Root Password <==
# Set a password for the 'root' user.
# IMPORTANT: Use a secure password, especially if the container
# will be exposed to any network. For local development, 'root' is simple.
RUN echo 'root:R00Tpassw*rd' | chpasswd
